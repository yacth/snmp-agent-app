...
void init(Mib& mib, UdpAddress& src)
{
	...
	UsmUserTable *uut = new UsmUserTable();
        ...
        // We do intentionally create all users as non-localized users too,
        // to be able to use them during request forwarding.
        // This has security implications you should be aware of. 
        // Use addNewRow(..., false) to be safe, but then additional 
        // configuration effort for proxy target users is necessary  
...
main (int argc, char* argv[])
{
	...
	mib = new Mib();
	requestList = new RequestList(mib);
#ifdef _SNMPv3
	// register v3MP
